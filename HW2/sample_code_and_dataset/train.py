...
from model import YourGNNModel,GATModel,StableGATModel,GCNEncoder,ContrastiveHead ,SSP_GCN # Build your model in model.py
...
if __name__ == '__main__':
    ...
    # 1) 載入預訓練
    enc = GCNEncoder(in_size, hid_size).to(device)
    enc.load_state_dict(torch.load("encoder_ssp.pth"))


    # model = YourGNNModel(in_size, 64, out_size).to(device)
    #model = StableGATModel(in_size, 32, out_size, num_heads=4).to(device)

    # 嘗試更大 hidden：
    #model = GATModel(in_size, 64, out_size, num_heads=8).to(device)

    #更小 hidden:
    #model = StableGATModel(in_size, 16, out_size, num_heads=2).to(device)

    # 2) 接分類器
    model = SSP_GCN(enc, hid_size, out_size).to(device)
